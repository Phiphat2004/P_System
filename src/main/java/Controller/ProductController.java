...
public class ProductController extends HttpServlet {
    ...
    if (request.getParameter("addNew") != null) {
        ...
        for (Size i : ls) {
            if (i.getSize().equalsIgnoreCase(size)) { // Kiểm tra đúng size
                if (i.getQuantity() >= quantity) { // Kiểm tra số lượng tồn kho
                    checkQuantity = true;
                    break; // Thoát khỏi vòng lặp nếu đã tìm thấy size phù hợp
                }
            }
        }
